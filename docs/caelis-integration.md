...
1. 在 Caelis 官方 `/connect` 流程配置模型。模型凭据由 Caelis 管理，不填入 Bot 设置。
2. Bot 的「设置 → 连接」选择 Caelis；可自动查找或指定可执行文件完整路径。
3. 「检测」读取 CLI 版本。未安装时可点击「安装 Caelis」，调用官方 HTTPS 安装脚本。
...
5. 「检测并保存」校验 Host 能力与既有身份。成功后下次启动生效，当前连接不被热替换。
   进行中工作、审批、未知结果会阻止切换。
