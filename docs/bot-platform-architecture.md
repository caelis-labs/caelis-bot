...
Codex 已适配，Caelis 通用协议待并行实现。Notebook/Memory、完整文件交付与新调度模型仍是目标。
...
- Memory 已有公开的 Go 嵌入接口、Remember/Recall、事实读取和治理能力。
  **当前 Caelis Bot 尚未嵌入它，也没有统一的个人记忆管理。**
...
| 数据 | 保存什么 | 权威与修改方式 |
| --- | --- | --- |
| Notebook | 长期目标、约定、资料、用户想保留的可读笔记 | Markdown 为正文真源；可查看、编辑、导出；写入检查 revision |
| Memory | 有来源的记忆证据、可检索片段、已确认事实及其变更 | 使用 Memory 公开 API；记录来源、时间、确认状态；可纠正、忘记 |
| 产品账本 | 工作进度、提醒、授权、交付状态、未知执行结果 | 结构化原生状态；模型不能通过写笔记将任务标为完成 |

不自动将每次笔记修改同步成另一份不可追踪的“记忆真相”。首版 Notebook 自身提供受限关键词检索；
只有明确记忆操作或选中的证据进入 Memory，并保留来源与版本。以后如增加笔记全文索引，
...
BotData/
  identity.json
  notebook/index.md + notes/
  memory/                   # 同一个 Bot 的记忆，由嵌入的 Memory 独占管理
...
整棵 `BotData` 不作为模型工作区。秘书使用笔记/记忆工具；worker 只获得自己的 `work/<task>`。
...
首版采用无需模型的基础检索。可选的 Steward/自动整理后置；不能为它在 Bot 偷存第二份 API Key，
也不能把模型推断自动升级为“用户已确认事实”。如果未来需要模型服务，必须另行定义复用 Runtime
的受控执行端口，不直接依赖 Caelis 内部包。

Memory 的 `Recall` 返回证据，不保证每条都是当前有效偏好。个人资料展示和启动摘要读取当前 Facts；
历史检索结果保留时间、来源与被替代状态。模型调用 `remember` 默认形成记忆证据，
只有宿主验证的明确用户声明/确认才能走相应可信证据路径，不能接受模型自报“用户确认”。

### 4.4 使用路径

1. 新一轮对话提供少量当前事实摘要与 Notebook 索引，不注入全部历史。
2. 需要旧信息时按需检索；读取结果作为新增上下文，普通笔记更新不改写已经发送的前缀。
3. 委派工作时生成有来源、可限长的上下文包，只提供与该工作有关的内容，不把共享记忆全量交给 worker。
4. 结果返回后由秘书决定是否提出记忆候选；执行回执和模型推断不自动写成个人事实。
5. 用户可从简洁的“记忆与笔记”入口查看、更正、忘记和导出，也可以在聊天中完成。
...
这些是审计与去重信息，不是记忆分区。Notebook 用 revision 防止覆盖；事实纠正通过 Memory 治理接口。
旧任务迟到的总结只能提交候选，不能覆盖后来明确纠正的个人事实。切换后的摘要从当前事实重建，
