...
启动时关怀文件不可读、内容损坏或规则无法重新编译，也只停用关怀：错误进入 Bot 状态、
工具结果和宿主诊断，个人空间与 Runtime 继续启动。保留原文件，不自动清空规则、水位或
未知派发记录；修复存储后重启再加载。
普通提醒的未知派发先按原始请求 ID 读取原生保留回执，再允许新的关怀激活。
只有精确匹配的接受回执成功写盘才解除等待；失败仍保留原未知状态，不重发。
...
应用回归覆盖损坏 JSON、不可编译规则、未知存储版本、无读取权限和非文件路径的启动隔离，
并检查原始字节保留。调度回归覆盖关怀到期与提醒恢复同轮发生、后续用户消息覆盖最新回执、
确认写盘失败、回执 ID 不符，以及恢复后继续交付关怀和下一次普通提醒。
