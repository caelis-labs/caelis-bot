...
2026-09-25 主动关怀审查修复（本地实现，尚未发布）：

- 启动时的关怀加载故障隔离到关怀模块；损坏 JSON、不可编译规则、未知版本、读取权限
  和非文件路径均保留原始存储，并通过 Bot 状态、工具错误和诊断报告，不中断个人空间与 Runtime。
- 普通提醒按原始请求 ID 恢复原生接受回执，恢复先于新的关怀派发；本地确认写盘失败
  不释放未知状态，后续消息或关怀不能覆盖恢复所需证据。
- 两个问题均先由新增回归复现，再验证修复。`make check`、`make smoke`、macOS ad-hoc
  `make build`、app/bot/care/Codex/Caelis race 和 Skill 校验通过。覆盖原字节保留、未知回执、
  错误 ID、写盘故障、同轮关怀到期及后续提醒继续运行。
- 本轮未重启日常 Bot，也未发送真实模型请求；启动隔离使用真实应用生命周期配合测试后端，
  调度交错由确定性测试验证，不替代发行版原生 GUI 与真实账户验收。

2026-09-25 任务历史、脚边关注清单与终端偏好（本地实现，尚未发布）：
...
- 打开终端只由用户点击触发。应用异步等待脚本回执并合并重复点击，不占用 Bot 回合、
  阻塞任务投影或代替 Runtime 就绪判断。右键可取消打开；取消、退出、五分钟超时撤销
  尚未执行的请求，迟到确认不再运行。系统接收打开请求不再等同于执行成功。
- `make check`、`make smoke`、macOS ad-hoc 构建、受影响包 race 和 Skill 校验通过。
  回归覆盖 105 条历史的分页稳定性、搜索、旧数据迁移、8 项上限、持久化失败、并发调整、
  续跑与回执核对、自动保存合并/CAS、卸载回退、自定义参数和延迟确认撤销。
- 实际 React 组件检查中英文、自动保存及重新挂载读回、缺失终端过滤和高级命令开关；
  AppKit fixture 检查 8 项滚动、右键移除、等待加载、取消入口和失败展示。
  Terminal/iTerm2/Ghostty 的生产启动参数均曾实际执行合成脚本。新增回执的最终原生轮次
  Terminal/Ghostty 通过；iTerm2 未在五分钟内手动确认，正确返回未确认，因此该轮确认成功
  分支未完成实测。没有重启日常 Bot 或发送真实模型请求；完整桌面点击与任意自定义终端
  不由上述夹具替代。详见[任务委派](task-delegation.md)。

2026-09-25 可编程主动关怀（本地实现，尚未发布）：

- `internal/care` 生产引擎和 `bot_care` 提供有界纯 CEL、来源发现、试算、规则保存/删除。
  首批为当前分钟、使用时长和前台应用切换；不读取窗口标题、屏幕或输入内容。
- 规则版本、来源水位、冷却、有效期、总预算和未知回执持久化；明确解锁且 Bot 空闲时，
  使用原有 Codex/Caelis 后台激活。Caelis 使用独立命名空间的原生 grant；关怀存储故障
  不阻断普通提醒。未知派发先读原生证据，不自动重发。
- 来源可通过宿主注册和应用内 JSON 发布接口扩展。已验证嵌套的 GitHub PR 示例数据进入
  CEL，但没有 gh/任意脚本执行器或外部连接器订阅。Bot Skill 新增按需加载的支持范围、
  三个内置场景及扩展示例；具体合同见[主动关怀](proactive-care.md)。
- `make check`、`make smoke`、`make build`（macOS ad-hoc）、受影响包 race、技能校验通过。关怀核心语句覆盖率 **89.2%**；
  测试覆盖类型/成本、时区/DST、容量、并发、预算、锁定/未知、授权替换、崩溃窗口、持久化
  失败、回执核对与扩展来源的应用生命周期。
- 安装版 Codex/Caelis + 本机合成 provider 验证 CEL 命中后的原生后台派发、Caelis 授权和
  静默投影；无真实账户/付费请求。生产 AppKit 采样 fixture 当前取得明确的 awake/unlocked。
  未重启日常 Bot，未主动锁屏/睡眠；物理切换、快速用户切换、OS 差异及真实模型效果仍待实测。

2026-09-25 聊天、任务气泡与 Bot 核心技能（本地实现，尚未发布）：

- 发送立即展示用户气泡，按原生请求身份与回执合并；拒绝和未知保留草稿，不自动重发。
  聊天和快捷输入均遵循 Runtime 的发送/运行中追加能力。Caelis 新 prompt 通过回执的回合
  目标关联，追加通过输入事件关联；消息先到、重复文本和恢复均不靠文本猜测去重。
- Codex 已完成的原生子回合不会再被迟到的父活动通知重新激活；真实新回合仍可正常运行。
- 委派 prompt 直接传递任务正文。英文 `caelis-bot-memory` 作为统一行为入口，根文件精简，
  记忆、任务、提醒和表达按需读取。两种 Runtime 仅收到应用范围的技能描述和路径，
  不自动注入正文，不安装到全局技能，也不传给 worker；AGENTS 已加入技能同步检查要求。
- 脚边任务气泡与预览在安装内容前设置窗口尺寸，修复裁切；活跃任务显示加载环，完成或
  隐藏时停止，减少动态效果时静止。悬停期间保持任务顺序，同时更新真实状态。
- `make check`、受影响包 race、`make smoke`、`make build` 通过。技能 frontmatter 校验通过。
  Codex/Caelis 安装版在隔离配置与本机合成 provider 下验证元数据、原生渐进读取和 worker
  隔离；Caelis 另验证运行中连续追加两条相同文本，各自有独立消息身份。
- 实际 React 组件以慢回执、接受和未知状态验证立即显示及追加输入；原生 AppKit fixture
  检查并截图确认展开布局、实时状态、加载动画和隐藏/减少动态效果。未重启日常 Bot，
  未调用真实付费模型；完整 Wails 跨窗口/输入法、跨屏 hover、外部 Terminal 点击和真实
  模型长期自主读取/压缩恢复仍需日常复验。详见[修复清单](bugfix-checklist.md)与
  [技能加载契约](design/bot-core-skill-v1.md)。
...
- “运行时与模型”统一 Bot 对话模型、Caelis 主模型、Agent team、连接和程序管理；权限单独成页。
  Team 支持原生角色、逐角色候选、推理/速度、自定义角色及方案；`self` 只读，配置写入保留 revision。
...
- Caelis 的 `WorkTerminalProvider` 已接入原生 Worker，使用同一 Session；主动关怀的后续产品化见上方 2026-09-25 更新。
  细节与复现命令见[任务委派](task-delegation.md)。

2026-09-24 设置、Dock 与安装界面整理（本地实现，尚未发布）：

- 设置分类切换回到顶部；快捷键读屏名称包含当前组合。模型与连接的当前交互见上方 Runtime 设置记录。
