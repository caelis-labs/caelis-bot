...
export function TeamSettings({ team, models, client, onRefresh }: { team: TeamState; models: ModelOption[]; client: RuntimeSettingsClient; onRefresh: () => Promise<void> }) {
 ...
 const row = (role: TeamRole) => <div className="runtime-setting-row" key={role.id}><div><strong title={role.description}>{role.id}</strong>{!role.modelIds && <p className="settings-note">请更新并重启 Caelis，以读取此角色的可用模型。</p>}{role.problem && <p className="inline-error">{role.problem}</p>}</div><div className="runtime-row-actions"><ModelSummary unbound value={role.selection} models={models} label={` ${role.id}`} onClick={() => setEditing(role)}/>{role.custom && <button className="text-action" onClick={() => { setEditing(role); open('delete-role'); }}>删除</button>}</div></div>;
 return <section className="runtime-team">
  ...
  {!team.available ? <p className="settings-note">{team.reason}</p> : <>
   ...
   <details className="runtime-system-agents"><summary>系统 Agent</summary><div className="runtime-setting-row"><strong>self</strong><span>跟随主模型</span></div>{team.roles.filter(r => r.system && r.id !== 'self').map(row)}</details>
  </>}
  {editing && !panel && <ModelPicker requireEffort title={`配置 ${editing.id}`} description={editing.description || undefined} value={editing.selection} models={models.filter(m => editing.modelIds?.includes(m.model))} onSave={selection => write({ action: 'bind', id: editing.id, selection })} onClose={() => setEditing(null)} onReload={onRefresh} onReset={!editing.inherited ? () => write({ action: 'reset', id: editing.id }) : undefined}/>}
