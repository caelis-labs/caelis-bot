...
export function RuntimeSettings() {
 ...
 return <section aria-label="连接设置">
  ...
  <SettingGroup title="本机运行时">
   <SettingRow label="运行时" htmlFor="runtime-provider"><select id="runtime-provider" value={settings.runtime} disabled={busy||!loaded} onChange={e=>setSettings({...settings,runtime:e.target.value})}><option value="codex">Codex</option></select></SettingRow>
   <SettingRow label="自动查找 Codex" htmlFor="runtime-automatic"><input id="runtime-automatic" className="settings-switch" role="switch" type="checkbox" checked={automatic} disabled={busy||!loaded} onChange={e=>{setAutomatic(e.target.checked);setMessage('');setError('');}}/></SettingRow>
   {!automatic&&<div className="setting-custom-path"><label htmlFor="runtime-path">Codex 路径</label><div className="runtime-path"><input id="runtime-path" aria-label="Codex CLI 路径" type="text" autoComplete="off" spellCheck={false} value={settings.cliPath} disabled={busy||!loaded} placeholder="输入可执行文件的完整路径" onChange={e=>{setSettings({...settings,cliPath:e.target.value});setMessage('');}}/><button disabled={busy||!loaded} onClick={()=>void pick()}>选择文件</button></div></div>}
  </SettingGroup>
  ...
  <SettingHelp><p>使用本机已登录的 Codex。自动模式优先连接本地服务，再查找 CLI；手动路径检测通过后保存。</p></SettingHelp>
 </section>;
}
