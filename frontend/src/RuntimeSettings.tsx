...
export function RuntimePreparation({onboarding=false,onDone,initialRuntime='',onBusy,call=backend,host=desktop}:{onboarding?:boolean;onDone?:()=>void;initialRuntime?:string;onBusy?:(busy:boolean)=>void;call?:typeof backend;host?:typeof desktop}) {
 ...
 const run=async(action:string,fields:Partial<SetupRequest>={})=>{
  ...
  try{const v=action==='detect'?await call<SetupState>('InspectSetup',profile):await call<SetupState>('ApplySetup',request(profile,action,fields));if(alive.current){setState(v);setForm('');if(action==='detect')setNotice(v.state==='ready'?'连接检测通过。':v.message);if(['install','update','apply-update','start'].includes(action))setNotice(id==='caelis'?'已连接':v.message);if(action==='check-update')setNotice(v.installation.updateState==='available'?'':v.message);if(profile.runtime===overview?.active&&(['login','api-key','logout'].includes(action)||profile.runtime==='codex'&&['update','install'].includes(action)))setRestartNeeded(true);if(profile.runtime==='caelis'&&['update','apply-update','start'].includes(action)&&v.state==='ready')setRestartNeeded(false);}}
 ...
 return <section className={onboarding?'runtime-onboarding':'runtime-management'} aria-label="运行时设置">
  ...
  <div className="runtime-program">
   <dl className="runtime-version-list">
    <div><dt>已安装</dt><dd>{state?.installation.installed ? state.installation.version : busy==='detect'?'正在检测…':'未安装'}</dd></div>
    {id==='caelis'&&state?.installation.installed&&<div><dt>运行中</dt><dd><span className={`runtime-status-dot ${state.state==='ready'||state.state==='models'?'ready':''}`} aria-hidden="true"/>{state.serviceState==='running'?state.serviceVersion||'版本未知':state.state==='service'?'未启动':'未连接'}</dd></div>}
   </dl>
   {serviceNeedsApply&&<p className="runtime-update-hint">{state?.state==='service'?'启动服务后即可使用。':'新版尚未启用。'}</p>}
   <div className="runtime-program-actions">
    {!state?.installation.installed?<button className="primary" disabled={blocked||!state} onClick={()=>void run('install')}>安装 {name}</button>:serviceNeedsApply?<button className="primary" disabled={blocked} onClick={()=>setConfirm(request(profile,'apply-update'))}>启用已安装版本</button>:updateAvailable?<button className="primary" disabled={blocked} onClick={()=>setConfirm(request(profile,'update'))}>{id==='caelis'?'更新并重新连接':'更新 Codex'}</button>:<button disabled={blocked} onClick={()=>void run('check-update')}>检查更新</button>}
    <button className="text-action" disabled={blocked||!id} onClick={()=>void run('detect')}>重新检测</button>
   </div>
   {updateAvailable&&<p className="settings-note">新版本 {state?.installation.latestVersion} 可用</p>}
  </div>
  ...
  <details className="runtime-advanced" open={manual||undefined}><summary>程序与数据目录</summary>
   <SettingGroup><SettingRow label="程序位置" description={state?.installation.path||'自动发现'}><button disabled={blocked} onClick={()=>{setManual(v=>!v);if(manual){setProfile(p=>({...p,cliPath:''}));setState(null);}}}>{manual?'使用自动发现':'更改'}</button></SettingRow>
    ...
    <SettingRow label="安装与卸载"><button onClick={()=>void call('OpenMessageLink',id==='caelis'?'https://caelis.dev':'https://learn.chatgpt.com/docs/codex/cli')}>查看官方指引</button></SettingRow>
   </SettingGroup>
  </details>
  {busy&&<p className="settings-note" role="status">{busy==='install'?'正在下载并安装…':busy==='update'?(id==='caelis'?'正在更新程序、启用服务并验证连接…':'正在更新 Codex…'):busy==='apply-update'?'正在启用服务并验证连接…':busy==='activate'?'正在准备重新启动…':'正在处理…'}</p>}
  {error?<p className="inline-error" role="alert">{error}</p>:notice?<p className="settings-note" role="status">{notice}</p>:state?.message&&!serviceNeedsApply&&!['ready','models'].includes(state.state)&&(state.state!=='incompatible'||onboarding||overview?.active===id)&&<p className="settings-note" role="status">{state.state==='incompatible'?'请更新 Caelis 后继续使用。':state.message}</p>}
  ...
  {confirm&&<section className="runtime-confirm" role="alertdialog" aria-modal="false" aria-labelledby="setup-confirm-title"><h3 id="setup-confirm-title">{confirm.action==='switch'?`切换到 ${name}？`:confirm.action==='logout'?'退出 Codex 登录？':confirm.action==='update'?(id==='caelis'?'更新并启用 Caelis？':'更新 Codex？'):confirm.action==='apply-update'?'启用已安装的 Caelis？':'移除此模型？'}</h3><p>{confirm.action==='switch'?'Caelis Bot 将重新启动，各运行时的对话和配置独立保留。':['update','apply-update'].includes(confirm.action)&&id==='caelis'?'其他 Caelis 窗口会暂时断开。请先结束其中的任务，更新期间不要开始新工作。检测到任务未结束或状态不明时，会暂停更新。':confirm.action==='update'?'更新 Codex 程序后，需要重新启动 Bot 才能启用。':'这会影响共享该运行时配置的其他应用。'}</p><div className="setup-end"><button autoFocus onClick={()=>setConfirm(null)}>取消</button><button className="primary" onClick={()=>{const r=confirm;setConfirm(null);if(r.action==='switch')void activate();else void run(r.action,r);}}>{confirm.action==='switch'?'切换并重启':confirm.action==='update'?(id==='caelis'?'更新并重新连接':'更新'):confirm.action==='apply-update'?'启用并重新连接':'确认'}</button></div></section>}
