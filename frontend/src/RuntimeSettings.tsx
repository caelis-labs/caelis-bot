...
export function RuntimeSettings({onboarding=false,onDone,onModels}:{onboarding?:boolean;onDone?:()=>void;onModels?:()=>void}) {
 ...
 return <section className={onboarding?'runtime-onboarding':'runtime-management'} aria-label="运行时设置">
  {onboarding?<><button className="text-action setup-back" disabled={blocked} onClick={()=>setID('')}>← 选择其他运行时</button><h1>准备 {name}</h1><p className="setup-lead">使用你电脑上的 {name}，或安装一个新版本。</p></>:<>
   <h1>连接</h1><p className="settings-intro">管理本机运行时和账号，模型选择集中在「模型与权限」。</p><SettingGroup><SettingRow label="当前使用" description={overview?.pending?`待切换到 ${names[overview.pending]}，重新启动后生效`:'各运行时保留独立的对话和设置'}><span>{names[overview?.active??'']??'—'}</span></SettingRow></SettingGroup>
  ...
  {!onboarding&&overview?.active===id&&state?.state==='ready'&&<p className="settings-note"><button className="text-action" onClick={onModels}>模型与权限设置 →</button></p>}
