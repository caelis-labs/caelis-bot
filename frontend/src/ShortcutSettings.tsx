...
export function ShortcutSettings(){
 ...
 return <SettingGroup title="聊天快捷键">
  <SettingRow label="全局唤出" description="打开聊天并聚焦输入区，再按一次隐藏" htmlFor="quick-shortcut"><input id="quick-shortcut" type="checkbox" role="switch" className="settings-switch" checked={value.enabled} disabled={!ready||busy} onChange={e=>void save({...value,enabled:e.target.checked})}/></SettingRow>
  ...
  }}>{recording?'按下快捷键…':label(value)}</button><button disabled={!ready||busy} onClick={()=>void save(initial)}>重置</button><button disabled={busy} onClick={()=>void desktop('ToggleHistory')}>试用</button></div></SettingRow>
